...
const STEPS = [
  { step: '01', title: '사고 후 내원', desc: '사고접수번호, 대인담당자 연락처, 상대방 보험회사명 세 가지만 준비하세요. 당일 내원 가능합니다.' },
  { step: '02', title: 'RMSK 초음파 정밀 진단', desc: '차의현 원장이 직접 RMSK 초음파로 경추·요추·근육·인대 손상 상태를 확인합니다.' },
  { step: '03', title: '맞춤 치료 시행', desc: '침·약침·추나·어혈한약을 자동차보험 100% 적용으로 치료합니다. 본인부담금 0원입니다.' },
  { step: '04', title: '회복까지 책임 진료', desc: '차의현 원장이 처음부터 끝까지 직접 진료합니다. 보험 청구 서류도 준비해드립니다.' },
]
...
export default function Home() {
  return (
    <>
      {/* HERO */}
      <section style={{ background: 'linear-gradient(135deg, #3d2a14 0%, #5c3d1e 60%, #8B6B3D 100%)', padding: '80px 24px 72px', textAlign: 'center' as const }}>
      ...
      <section style={{ background: '#fdf9f4', borderBottom: '1px solid #e4e1d9', padding: '20px 24px' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'flex', justifyContent: 'center', gap: '40px', flexWrap: 'wrap' as const }}>
          {['✅ 자동차보험 100% 적용', '✅ 본인부담금 0원', '✅ 당일 내원 가능', '✅ RMSK 초음파 진단', '✅ 차의현 원장 직접 진료'].map(s => (
      ...
      <section style={{ padding: '72px 24px', background: '#fff' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '2px' }}>
            {CLINICS.map((c, i) => (
              <a key={c.title} href={c.href} style={{ background: i % 2 === 0 ? '#f7f6f3' : '#fff', border: '1px solid #e4e1d9', padding: '32px 24px', display: 'block', textDecoration: 'none' }}>
      ...
      <section style={{ padding: '72px 24px', background: '#f7f6f3' }}>
        <div style={{ maxWidth: '780px', margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: '40px', alignItems: 'flex-start' as const, flexWrap: 'wrap' as const }}>
            <img src="https://orbitchae.com/images/doctor-profile.jpg" alt="목동교통사고한의원 오비채한의원 차의현 원장"
              style={{ width: '200px', height: '200px', objectFit: 'cover' as const, borderRadius: '8px', flexShrink: 0 }} />
            <div style={{ flex: 1, minWidth: '200px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '8px' }}>
                {['RMSK 미국 근골격계 초음파 자격', '현대고방학회 복진 실습 강의', '교통사고 후유증 전문 진료', '자동차보험 100% 직접 청구'].map((item, i) => (
      ...
      <section style={{ padding: '72px 24px', background: '#fff' }}>
        <div style={{ maxWidth: '780px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '2px' }}>
            {STEPS.map((item, i) => (
              <div key={item.step} style={{ background: i % 2 === 0 ? '#fff' : '#f7f6f3', border: '1px solid #e4e1d9', padding: '24px 28px', display: 'flex', gap: '20px', alignItems: 'flex-start' as const }}>
      ...
      <section style={{ padding: '72px 24px', background: '#f7f6f3' }}>
        <div style={{ maxWidth: '780px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center' as const, marginBottom: '40px' }}>
            ...
            <h2 style={{ fontFamily: 'Georgia, serif', fontSize: '28px', fontWeight: 400, color: '#161616' }}>오시는길</h2>
          </div>
      ...
      {/* CTA */}
      <section style={{ background: 'linear-gradient(135deg, #3d2a14, #5c3d1e)', padding: '72px 24px', textAlign: 'center' as const }}>
        <div style={{ maxWidth: '780px', margin: '0 auto' }}>
